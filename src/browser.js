...
ContentService.prototype._exportDecryption = function (): File {
...
ContentService.prototype._exportHashes = function (): File {
...
MetadataService.prototype._exportHashes = function (): File {
  const hashes = this._hashes
  if (!hashes) {
    throw constellate.ErrNoHashes
  }
...
MetadataService.prototype._exportLinkedData = function (): File {
  const ld = this._ld
  if (!ld || !ld.length) {
...
MetadataService.prototype.importLinkedData = function (file: File, cb: Function) {
  if (file.type !== 'application/json') {
    return cb(errUnexpectedType(file.type, 'application/json'))
  }
  const tasks = new Tasks(cb)
...
Project.prototype.import = function (content: File[], metadata: File, password: string, cb: Function) {
